...
from cornac.models import BiVAECF, BPR
import pickle
...
DATA_PATH = "train.txt"  
OUTPUT_PATH = "hybrid_bivae_bpr_top20.txt"
MODEL_BIVAE_PATH = "bivae_model.pkl"
MODEL_BPR_PATH = "bpr_model.pkl"
...
print("Starting BiVAE + BPR experiment...")
...
bivae.save(MODEL_BIVAE_PATH)
bpr.save(MODEL_BPR_PATH)
print(f"Saved BiVAE model to: {MODEL_BIVAE_PATH}")
print(f"Saved BPR model to:   {MODEL_BPR_PATH}")
...
print(f"Generating hybrid (0.7 BiVAE + 0.3 BPR) top-20 recommendations into: {OUTPUT_PATH}")
...
with open(OUTPUT_PATH, "w") as f_out:
    for u_idx in range(num_users):
        ...
        scores_bivae = bivae.score(u_idx)  
        scores_bpr = bpr.score(u_idx)      

        hybrid_scores = 0.7 * scores_bivae + 0.3 * scores_bpr

        user_interactions = set(train_set.matrix[u_idx].indices)

        candidates = [
            (i, hybrid_scores[i]) for i in range(num_items)
...
print("Done. Hybrid recommendation file written:")
print(f"  {OUTPUT_PATH}")
